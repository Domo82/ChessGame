...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
	...
	*/
	...
	private String getPieceName(int x, int y) {
		Component cl = chessBoard.findComponentAt(x,y);
		if (cl instanceof JPanel) {
			return "empty";
		} else if (cl instanceof JLabel) {
			JLabel awaitingPiece = (JLabel) cl;
    ...
    System.out.println("-------------------------------------------------");
		...
		*/
		if(pieceName.contains("Bishup")) {
			...
			Boolean inTheWay = false;
			if (((landingX > 7) || (landingX < 0)) || ((landingY > 7) || (landingY < 0 ))) {
				validMove = false;
			} else if (!pieceMove(landingX, landingY)) {
				validMove = false;
					...
					if (pieceName.contains("White")) {
						if (checkWhiteOponent(e.getX(), e.getY())) {
							...
							if (getPieceName(e.getX(), e.getY()).contains("King")) {
								...
								System.exit(0);
								winner = "The White player has won the game!";
							}
						} else {
							validMove = false;
						}
			...
			if((xMovement == 0) && (yMovement == 0)) {
				...
				validMove = true;
			} else if(pieceMove(e.getX(),e.getY())) {
				if (pieceName.contains("White")) {
					if (checkWhiteOponent(e.getX(),e.getY())) {
						validMove = true;
					}
					...
					validMove = true;
						...
						if (checkWhiteOponent(e.getX(),e.getY())) {
							...
							if (getPieceName(e.getX(), e.getY()).contains("King")) {
								System.out.println("Black player has won the game");
								System.exit(0);
								winner = "Black player has won the game";
					}
				}
			}
		}
